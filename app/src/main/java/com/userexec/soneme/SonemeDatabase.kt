...
class SonemeDatabase(context: Context) : SQLiteOpenHelper(context, "soneme.db", null, 1) {
    ...
    fun getAudio(uri: String): AudioRecord? = readableDatabase.rawQuery(
        "SELECT uri,title,artist,duration_ms,position_ms,last_played FROM audio_state WHERE uri=?",
        arrayOf(uri)
    ).use { c ->
    ...
    fun recents(): List<AudioRecord> = readableDatabase.rawQuery(
        "SELECT uri,title,artist,duration_ms,position_ms,last_played FROM audio_state " +
